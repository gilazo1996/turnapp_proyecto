...
abstract class BaseListView extends Widget
{
    ...
    public function init()
    {
        ...
        if ($this->emptyText === null) {
            $this->emptyText = Yii::t('yii', 'No hay resultados.');
        }
    ...
    public function renderSummary()
    {
        ...
        if (($pagination = $this->dataProvider->getPagination()) !== false) {
            ...
            $page = $pagination->getPage() + 1;   //'Showing <b>{begin, number}-{end, number}</b> of <b>{totalCount, number}</b> {totalCount, plural, one{item} other{items}}.'
            $pageCount = $pagination->pageCount;
            if (($summaryContent = $this->summary) === null) {
                return Html::tag($tag, Yii::t('yii', 'Mostrando <b>{begin, number}-{end, number}</b> of <b>{totalCount, number}</b> {totalCount, plural, one{item} other{items}}.', [
                        'begin' => $begin,
                        'end' => $end,
                        'count' => $count,
